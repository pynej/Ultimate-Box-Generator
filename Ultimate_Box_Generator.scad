echo("Built using Ultimate Box Generator v3.10.0");
...
lid_type_1_enhanced_snaps_pertrusion_factor = 0.63; // Shift the enhanced snaps to the left or right. Higher values make it more lose, lower values make it tighter but risk the side-walls of the box breaking off. You probably want this to be between 0.55 and 0.67.
    ...
    difference() {
        ...
        if(lid_type==1 || lid_type == 4) {
            ...
            if(lid_type_1_enhanced_snaps) {
                translate([wall * 1.5, wall * (0.5 + lid_type_1_enhanced_snaps_pertrusion_factor), totalheight + wall/2]) sphere(wall/2, $fn=lid_fn);
                translate([wall * 1.5, box_y - wall * (0.5 + lid_type_1_enhanced_snaps_pertrusion_factor), totalheight + wall/2]) sphere(wall/2, $fn=lid_fn);
            }
        }
        ...
        if(lid_type==4) {
            difference() {
                ...
                union() {
                    ...
                    if(has_snap) {
                        if (lid_type_1_enhanced_snaps) {
                            translate([wall * 1.5, wall * lid_type_1_enhanced_snaps_pertrusion_factor, wall/2]) sphere(wall/2, $fn=lid_fn);
                            translate([wall * 1.5, box_y - 2*tolerance - wall - wall * lid_type_1_enhanced_snaps_pertrusion_factor, wall/2]) sphere(wall/2, $fn=lid_fn);
                        } else {
                            polyhedron([
                                [0, -snap_inset, 0],
